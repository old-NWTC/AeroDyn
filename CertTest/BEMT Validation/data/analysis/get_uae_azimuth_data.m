function [az_vec, th_vec_all, tq_vec_all] = get_uae_azimuth_data(dir, wind_speed, yaw)
        ...
        filenum = ['H', num2str(wind_speed, '%02d'), ...
            num2str(yaw, '%04d'), '0'];
        hd_file = [dir, filenum, '.HD1'];
        eng_file = [dir, filenum, '.ENG'];

        % get number of channels and samples
        fid = fopen(hd_file);
        if (fid < 0) 
            fprintf('%s%s\n','Could not open file: ',hd_file, '\n');
            return 
        end
        ...
        fid = fopen(hd_file);
        if (fid < 0) 
            fprintf('%s%s%s','Could not open file: ',hd_file, '\n');
            return
        end
