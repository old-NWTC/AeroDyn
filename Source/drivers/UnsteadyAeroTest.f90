...
program UnsteadyAeroTest
   ...
   dt = .05_DbKi
   n  = 1
   InitInData%nNodesPerBlade  = 5 
   InitInData%numBlades       = 1
   
      ! All nodes/blades are using the same 2D airfoil
   afNames(1) = './af_files2/S809_CLN_298.DAT'
   
   
      ! Initialize the Airfoil Info Params
   call Init_AFI(NumAFfiles, afNames, AFI_Params, errStat, errMsg)
   ...
   do n = 1, 150
      ...
      do j = 1,InitInData%numBlades
         do i = 1,InitInData%nNodesPerBlade
            ...
            call UA_CalcOutput(u(1),  p, xd, OtherState, AFI_Params%AFInfo(AFIndx(i,j)), y, errStat, errMsg )
