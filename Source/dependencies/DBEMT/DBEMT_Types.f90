...
! =========  DBEMT_OtherStateType  =======
  TYPE, PUBLIC :: DBEMT_OtherStateType
    REAL(SiKi)  :: DummyOtherState      !< Remove this variable if you have other states [-]
...
! =========  DBEMT_InputType  =======
  TYPE, PUBLIC :: DBEMT_InputType
    REAL(ReKi)  :: AxInd_disk      !< Disk-averaged axial induction [-]
    REAL(ReKi)  :: Un_disk      !< Disk-averaged normal relative inflow velocity [m/s]
    REAL(ReKi)  :: R_disk      !< Disk-averaged rotor radius [m]
...
! 
   ErrStat = ErrID_None
   ErrMsg  = ""
IF (ALLOCATED(SrcMiscData%areStatesInitialized)) THEN
  i1_l = LBOUND(SrcMiscData%areStatesInitialized,1)
  i1_u = UBOUND(SrcMiscData%areStatesInitialized,1)
  i2_l = LBOUND(SrcMiscData%areStatesInitialized,2)
  i2_u = UBOUND(SrcMiscData%areStatesInitialized,2)
  IF (.NOT. ALLOCATED(DstMiscData%areStatesInitialized)) THEN 
    ALLOCATE(DstMiscData%areStatesInitialized(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstMiscData%areStatesInitialized.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstMiscData%areStatesInitialized = SrcMiscData%areStatesInitialized
ENDIF
...
! 
  ErrStat = ErrID_None
  ErrMsg  = ""
IF (ALLOCATED(MiscData%areStatesInitialized)) THEN
  DEALLOCATE(MiscData%areStatesInitialized)
ENDIF
...
! 
   ...
   ErrMsg  = ""
    ...
    DstParamData%c5 = SrcParamData%c5
    DstParamData%c6 = SrcParamData%c6
    DstParamData%c7 = SrcParamData%c7
    DstParamData%c8 = SrcParamData%c8
    DstParamData%c9 = SrcParamData%c9
